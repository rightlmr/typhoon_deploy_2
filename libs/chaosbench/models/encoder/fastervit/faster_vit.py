...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, in_chans=3, in_dim=64, dim=96):
        ...
        self.conv_down = nn.Sequential(
            nn.Conv2d(in_chans, in_dim, 3, 2, 1, bias=False), # stride=2，下采样2倍
            nn.BatchNorm2d(in_dim, eps=1e-4),
            nn.ReLU(),
            nn.Conv2d(in_dim, dim, 3, 1, 1, bias=False), # stride=1，不下采样
...
class FasterViT(nn.Module):
    ...
    def __init__(self,
        ...
        output:
            x: output tensor of shape (B, num_classes)
        """
        super().__init__()
...
if __name__ == "__main__":
    model = FasterViT(dim=64, in_dim=64, depths=[2, 2, 6, 2], window_size=[7, 7, 7, 7], ct_size=1, mlp_ratio=4., num_heads=[2, 4, 8, 16], resolution=224)
